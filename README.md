...
1. **configuration.yaml** - Komplett konfigurasjon med alt du trenger
2. **automations.yaml** - Alle automasjoner med smart kapasitetsledd-logikk
...
### STEG 2: Erstatt filene (2 min)

**Via SSH/Terminal:**
```bash
# Kopier de nye filene fra dette repoet til /config/
cp configuration.yaml /config/configuration.yaml
cp automations.yaml /config/automations.yaml
```

**Via File Editor (anbefalt):**
1. Åpne `configuration.yaml` i File Editor
2. SLETT alt innhold
3. Åpne `configuration.yaml` fra dette repoet
4. KOPIER alt innhold
5. LIM INN i din `configuration.yaml`
6. LAGRE

Gjenta for `automations.yaml`.
